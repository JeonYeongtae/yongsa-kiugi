...
function SkillView({ equippedWeaponCat, equippedSkillIds, onEquip, onBack }) {
...
function CompanionWeaponView({ companion, equippedId, allEquippedWeaponIds, onEquip, onBack }) {
  const [selected,    setSelected]    = useState(null);
  const [tierFilter,  setTierFilter]  = useState('전체');

  const catWeapons     = WEAPONS.filter(w => w.cat === companion.weapon);
  const availableTiers = ['전체', ...new Set(catWeapons.map(w => w.tier))].sort((a, b) => a === '전체' ? -1 : a - b);
  const filtered       = catWeapons.filter(w => tierFilter === '전체' || w.tier === tierFilter);

  if (selected) {
    const isMine       = selected.id === equippedId;
      ...
      <div className="flex-1 flex flex-col overflow-hidden bg-slate-100 relative">
        ...
        <div className="p-3 flex flex-col gap-3 flex-1">
          ...
          {takenByOther ? (
            ...
            <button onClick={() => onEquip(null)} className="w-full py-1.5 bg-red-100 text-red-600 text-[9px] rounded font-bold">
              해제하기 ✕
            </button>
          ) : (
            <button onClick={() => onEquip(selected.id)} className="w-full py-1.5 bg-slate-600 text-white text-[9px] rounded font-bold">
              장착하기 ▸
            </button>
          )}
        </div>
  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden bg-slate-100 relative">
      ...
      <div className="px-3 py-1.5 bg-slate-50 border-b border-slate-200 flex-shrink-0">
        <div className="text-[7px] text-slate-400">
          {CAT_ICON[companion.weapon]} {companion.weaponName} 전용 &nbsp;·&nbsp; 이미 장착된 무기는 선택 불가
        </div>
      </div>
      <div className="flex flex-wrap gap-1 px-3 pt-2 flex-shrink-0">
        {availableTiers.map(t => (
          <button key={t} onClick={() => setTierFilter(t)}
            className={`px-1.5 py-0.5 rounded-full text-[7px] transition-colors ${
              tierFilter === t ? 'bg-amber-400 text-white font-bold' : 'bg-slate-200 text-slate-400'
            }`}>
            {t === '전체' ? '전체' : TIER_LABEL[t]}
          </button>
        ))}
      ...
      <div className="flex-1 overflow-y-auto space-y-1 p-3">
        {filtered.map(w => {
          const isMine       = w.id === equippedId;
          const takenByOther = allEquippedWeaponIds.includes(w.id) && !isMine;
          return (
            <ListCard key={w.id}
              icon={CAT_ICON[w.cat]}
              tier={'★'.repeat(w.tier)}
    ...
    <div className="flex-1 flex flex-col overflow-hidden bg-slate-100 relative">
      ...
      <div className="grid grid-cols-4 gap-1.5 mb-2 flex-shrink-0">
        ...
        {Array(3).fill(0).map((_, i) => {
          ...
          const compWeapon = comp && partyEquipment[comp.id]
            ? WEAPONS.find(w => w.id === partyEquipment[comp.id])
            : null;
          return (
            <div key={i}
      ...
      <div className="grid grid-cols-4 gap-1.5 flex-shrink-0">
        ...
        {Array(3).fill(0).map((_, i) => {
          const comp   = party[i] != null ? COMPANIONS.find(c => c.id === party[i]) : null;
          const weapon = comp && partyEquipment[comp.id] ? WEAPONS.find(w => w.id === partyEquipment[comp.id]) : null;
          if (!comp) return (
...
function MainView({ equippedWeapon, equippedSkills, party, onWeapon, onSkill, onParty }) {
  return (
    <div className="flex-1 overflow-y-auto flex flex-col gap-4 p-3">
      ...
      <div>
        ...
        <div className="grid grid-cols-2 gap-1.5">
          {[0, 1].map(i => {
            ...
            return s ? (
              <button key={s.id} onClick={onSkill}
              ...
              <button key={`empty-${i}`} onClick={onSkill}
      ...
      <div>
        ...
        <button onClick={onParty} className="w-full grid grid-cols-4 gap-1.5">
...
const CONFIRM_MSG = {
  load:   { label: '불러오기', desc: '저장된 세팅을 현재 편성에 적용합니다.\n현재 편성이 변경됩니다.', btnClass: 'bg-slate-600 text-white' },
  save:   { label: '덮어쓰기', desc: '현재 편성으로 이 슬롯을 덮어씁니다.\n기존 세팅이 사라집니다.',  btnClass: 'bg-amber-400 text-white' },
  delete: { label: '삭제',     desc: '이 슬롯의 세팅을 삭제합니다.',                               btnClass: 'bg-rose-400 text-white'  },
...
export default function FormationScreen() {
  ...
  const [partyEquipment,     setPartyEquipment]     = useState({});
  ...
  const allEquippedWeaponIds = [
    equippedWeaponId,
    ...party.map(cId => partyEquipment[cId]).filter(id => id != null),
  ];
  ...
  const loadPreset = (i) => {
    const p = presets[i];
    if (!p) return;
    setEquippedWeaponId(p.weaponId);
  ...
  const renderContent = () => {
    ...
    if (view === 'companion-weapon' && companionForWeapon) return (
      <CompanionWeaponView
        companion={companionForWeapon}
        equippedId={partyEquipment[companionForWeapon.id] ?? null}
        allEquippedWeaponIds={allEquippedWeaponIds}
        onEquip={(weaponId) => {
          setPartyEquipment(prev => ({ ...prev, [companionForWeapon.id]: weaponId }));
          setView('party');
          setCompanionForWeapon(null);
        }}
        onBack={() => { setView('party'); setCompanionForWeapon(null); }}
      />
    );
    if (view === 'party') return (
      <PartyView
        ...
        onCompanionWeaponTap={(comp) => { setCompanionForWeapon(comp); setView('companion-weapon'); }}
        heroWeapon={equippedWeapon}
        onHeroWeaponTap={() => setView('weapon')}
        onBack={() => setView('main')}
      />
    );
    return (
      <MainView
        ...
        party={party}
        onWeapon={() => setView('weapon')}
        onSkill={() => setView('skill')}
        onParty={() => setView('party')}
      />
    );
  };
